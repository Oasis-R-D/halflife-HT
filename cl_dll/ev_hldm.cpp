...
void EV_FireGlock1(event_args_t* args)
{
	...
	if (team == true)
	{
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/steyr_fire1.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong(0, 3));
	}
	else
	{
	...
	EV_HLDM_FireBullets(idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_GLOCK, 0, &tracerCount[idx - 1], args->fparam1, args->fparam2);
}
...
void EV_FireMP5(event_args_t* args)
{
	...
	if (ismed == true)
	{
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/m45_fire1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
		EV_HLDM_FireBullets(idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_GLOCK, 2, &tracerCount[idx - 1], args->fparam1, args->fparam2);
	}
	else
	{
...
//	    AG36 START
//======================
void EV_FireAG36(event_args_t* args)
{
	int idx;
	Vector origin;
	Vector angles;
	...
	if (EV_IsLocal(idx))
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(AG36_FIRE1 + gEngfuncs.pfnRandomLong(0, 2), 0);
...
void EV_FireAG36_2(event_args_t* args)
{
	int idx;
	Vector origin;
	...
	if (EV_IsLocal(idx))
	{
		gEngfuncs.pEventAPI->EV_WeaponAnimation(AG36_LAUNCH, 0);
		V_PunchAxis(0, -10);
	}
