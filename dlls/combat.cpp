...
void CBaseEntity::FireBullets(unsigned int cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t* pevAttacker)
{
	...
	for (unsigned int iShot = 1; iShot <= cShots; iShot++)
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			if (0 != iDamage)
			{
				pEntity->TraceAttack(pevAttacker, iDamage, vecDir, &tr, DMG_BULLET | ((iDamage > 16) ? DMG_ALWAYSGIB : DMG_NEVERGIB) | DMG_GIBHEADS);
				...
				case BULLET_PLAYER_357:
					pEntity->TraceAttack(pevAttacker, gSkillData.plrDmg357, vecDir, &tr, DMG_BULLET | DMG_GIBHEADS);

					break;

				case BULLET_PLAYER_BUCKSHOT:
					// make distance based!
					pEntity->TraceAttack(pevAttacker, gSkillData.plrDmgBuckshot, vecDir, &tr, DMG_BULLET | DMG_GIBHEADS);
				...
				case BULLET_MONSTER_12MM:
					pEntity->TraceAttack(pevAttacker, gSkillData.monDmg12MM, vecDir, &tr, DMG_BULLET | DMG_GIBHEADS);
					TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
					DecalGunshot(&tr, iBulletType);
				...
				case BULLET_PLAYER_M249:
					pEntity->TraceAttack(pevAttacker, gSkillData.plrDmg556, vecDir, &tr, DMG_BULLET | DMG_GIBHEADS);
					TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
					DecalGunshot(&tr, iBulletType);
				...
				case BULLET_PLAYER_SNIPER:
					pEntity->TraceAttack(pevAttacker, gSkillData.plrDmg762, vecDir, &tr, DMG_BULLET | DMG_GIBHEADS);
					TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
					DecalGunshot(&tr, iBulletType);
				...
				case BULLET_PLAYER_EAGLE:
					pEntity->TraceAttack(pevAttacker, gSkillData.plrDmgEagle, vecDir, &tr, DMG_BULLET | DMG_GIBHEADS);
		...
		if (tr.flFraction != 1.0)
		{
			...
			if (0 != iDamage)
			{
				...
				case BULLET_PLAYER_AG36:
					pEntity->TraceAttack(pevAttacker, gSkillData.plrDmgAG36, vecDir, &tr, DMG_BULLET | DMG_GIBHEADS);
					break;
